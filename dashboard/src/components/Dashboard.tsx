...
const Dashboard = ({ onSiteSelect }: DashboardProps) => {
    ...
    const loadData = async () => {
        try {
            ...
            // Load sites config - for now, let's hardcode the sites to avoid YAML parsing issues
            const hardcodedSites = [
                {
                    name: 'golden-beach-villas',
                    type: 'wordpress',
                    url: 'https://goldenbeachvillas.com/',
                    perf_budget: {
                        lcp_ms: 2500,
                        tbt_ms: 300,
                        perf_score: 80,
